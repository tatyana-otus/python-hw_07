...
class LogisticRegression:
    ...
    def predict_proba(self, X, append_bias=False):
        ...
        ###########################################################################
        y_pos = sig(X, self.w)
        y_proba = np.vstack((y_pos, 1.0 - y_pos))
        ###########################################################################
        #                           END OF YOUR CODE                              #
        ###########################################################################
        return y_proba.T
    ...
    def predict(self, X):
        ...
        y_pred = np.where((y_proba[:, 0]) > 0.5, 1, 0)
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        loss = (1.0 / num_train) * loss
        dw = (1.0 / num_train) * dw

        # Add regularization to the loss and gradient.
        # Note that you have to exclude bias term in regularization.
        loss = loss + (2.0 / num_train) * np.sum(self.w[:dim-1]**2) * reg
        dw[:dim-1] = dw[:dim-1] + (reg / num_train) * self.w[:dim-1]
